...
function HomePage() { // Define HomePage functional component
  ...
  useEffect(() => { // useEffect hook to fetch properties data when component mounts
  ...
  const handleSearch = (filteredProperties) => { // Function to handle search filtering
    setFilteredProperties(filteredProperties); // Update filteredProperties state with filtered data
  };

  const handleDeleteProperty = (id) => { // Function to handle property deletion
    fetch(`http://localhost:3000/property/${id}`, { // Send DELETE request to delete property
      method: 'DELETE' // Specify request method
    })
    .then(() => { // Handle successful deletion
      const updatedProperties = properties.filter(property => property.id !== id); // Filter out deleted property
