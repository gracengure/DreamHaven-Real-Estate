...
function CreateProperty() { // CreateProperty functional component
  ...
  useEffect(() => { // useEffect hook to fetch existing properties when component mounts
    const fetchData = async () => { // Fetch function to get data from the backend
      try {
  ...
  const handleSubmit = async (event) => { // Function to handle form submission
    ...
    try {
      const response = await fetch('http://localhost:3000/property', { // Send POST request to create property
        method: 'POST',
        headers: {
      ...
      setLocation('');
      setBedrooms('');
    } catch (error) { // Catch any errors during property creation
  ...
  return (
    <div>
      <button onClick={() => setShowForm(!showForm)} style={{ textAlign: 'center', height: '34px', color: 'white', backgroundColor: '#01b0f8', borderRadius: '5px', border: 'transparent' }}>
        ...
        <div>
          <div className='cardBar'>
            ...
            <form onSubmit={handleSubmit}> 
              ...
              <br />
              <label>                Bedrooms:
                <input type="number" value={bedrooms} onChange={(e) => setBedrooms(e.target.value)} />            </label>
              <br />
      ...
      <div className='container'>        {properties.map((property, index) => ( // Map through properties array to render each property
